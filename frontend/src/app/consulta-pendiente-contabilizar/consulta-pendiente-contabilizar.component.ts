...
@Component({
  selector: 'app-consulta-pendiente-contabilizar',
  imports: [ CommonModule ,FormsModule, SidebarComponent],
  providers: [DatePipe],
  templateUrl: './consulta-pendiente-contabilizar.component.html',
  styleUrls: ['./consulta-pendiente-contabilizar.component.css']
})

export class ConsultaPendienteContabilizarComponent {
  //3 dots menu 
  showMenu = false;
  ...
  fetchFacturas() {
    ...
    this.http.get<any>(`${environment.backendUrl}/api/fde/fetch-pendiente-del-contabilizar/${this.entcod}/${this.eje}`).subscribe({
      next: (res) => {
  ...
  sortField: 'facnum' | 'facann' | 'facfac' | 'facdoc' | 'facdat' | 'fdeorg' | 'fdefun' | 'fdeeco' | 'limporte' | 'ternif' | 'ternom' |'cgecod' | 'cgecod' | null = null;
  sortColumn: string = '';
  ...
  search() {
    ...
    this.http.get<any>(`${environment.backendUrl}/api/fde/search-pendiente-contabilizar`, { params }).subscribe({
      next: (res) => {
        this.isLoading = false;
        this.facturas = res;
        this.updatePagination();
  ...
  downloadExcel() {
    ...
    XLSX.utils.sheet_add_aoa(worksheet, [['Número', 'Año Factura', 'Nº Registro C.F', 'Referencia Fact', 'Fecha Factura', 'Orgánica', 'Programa', 'Económica', 'Importe', 'NIF', 'Nombre', 'Centro Gestor']], { origin: 'A2' });
