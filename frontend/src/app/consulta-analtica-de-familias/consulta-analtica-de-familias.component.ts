...
@Component({
  selector: 'app-consulta-analtica-de-familias',
  standalone: true,
  imports: [ CommonModule ,FormsModule, SidebarComponent],
  templateUrl: './consulta-analtica-de-familias.component.html',
  styleUrls: ['./consulta-analtica-de-familias.component.css']
})
export class ConsultaAnalticaDeFamiliasComponent {
  //3 dots menu 
  showMenu = false;
  ...
  fatchFamilias() {
    ...
    this.http.get<any>(`${environment.backendUrl}/api/afa/fetch-familia-analitica/${this.entcod}`).subscribe({
      next: (res) => {
      ...
      error: (err) => {
        this.isLoading = false;
        this.familiaError = err.error.error || err.error;
      }
    })
  }
  ...
  get paginatedFamilias(): any[] {if (!this.familias || this.familias.length === 0) return []; const start = this.page * this.pageSize; return this.familias.slice(start, start + this.pageSize);}
  get totalPages(): number {return Math.max(1, Math.ceil((this.familias?.length ?? 0) / this.pageSize));}
  ...
  DownloadPDF() {
    ...
    doc.save('Consulta_analîtica_de_familias.pdf');
  }
  ...
  downloadExcel() {
    ...
    XLSX.utils.sheet_add_aoa(worksheet, [['Consulta analîtica de familias']], { origin: 'A1' });
    worksheet['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 3 } }];
    XLSX.utils.sheet_add_aoa(worksheet, [['Ejer/Periodo', 'Familias', 'Descripción']], { origin: 'A2' });
    ...
    saveAs(
      ...
      'Consulta_analîtica_de_familias.xlsx'
    );
  }
