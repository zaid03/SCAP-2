...
public class HistoricaADContratoSearch {
    ...
    public List<SaldoContrato> historicaADContratoSearch(
        Integer ent,
        String eje,
        String cge,
        String contrato,
        ...
        if ((proveedor != null && !proveedor.isBlank()) && (cge == null || cge.isBlank()) && (contrato == null || contrato.isBlank())) {
            if (isNumbersOnly(proveedor)) {
                ...
                contratos = cogRepository.findByENTAndEJEAndCot_conn_CONTIPAndCot_ter_TERCODOrENTAndEJEAndCot_conn_CONTIPAndCot_ter_TERNIFContaining(ent, eje, 3, tercod, ent, eje, 3, proveedor);
                return contratos;
            } else {
                contratos = cogRepository.findByENTAndEJEAndCot_conn_CONTIPAndCot_ter_TERNOMContainingOrENTAndEJEAndCot_conn_CONTIPAndCot_ter_TERNIFContaining(ent, eje, 3, proveedor, ent, eje, 3, proveedor);
                return contratos;
            }
        }
        ...
        if (cge != null && !cge.isBlank()) {
            if (contrato != null && !contrato.isBlank()) {
                if (isNumbersOnly(contrato)) {
                    ...
                    contratos = cogRepository.findByENTAndEJEAndCot_conn_CONTIPAndCge_CGECODAndCONCOD(ent, eje, 3, cge, concod);
                } else {
                    contratos = cogRepository.findByENTAndEJEAndCot_conn_CONTIPAndCge_CGECODAndCot_conn_CONDESContaining(ent, eje, 3, cge, contrato);
                }
                ...
                contratos = cogRepository.findByENTAndEJEAndCot_conn_CONTIPAndCge_CGECOD(ent, eje, 3, cge);
                if (proveedor != null && !proveedor.isBlank()) {
                    if (isNumbersOnly(proveedor)) {
                        contratos = filterTodosByTercodAndTernif(contratos, proveedor);
                    } else {
        ...
        if ((contrato != null && !contrato.isBlank()) && (cge == null || cge.isBlank())) {
            if (isNumbersOnly(contrato)) {
                contratos = cogRepository.findByENTAndEJEAndCot_conn_CONTIPAndCONCOD(ent, eje, 3, Integer.parseInt(contrato));
                if (proveedor != null && !proveedor.isBlank()) {
                    if (isNumbersOnly(proveedor)) {
                        contratos = filterTodosByTercodAndTernif(contratos, proveedor);
                    } else {
                ...
                contratos = cogRepository.findByENTAndEJEAndCot_conn_CONTIPAndCot_conn_CONDESContaining(ent, eje, 3, contrato);
                if (proveedor != null && !proveedor.isBlank()) {
                    if (isNumbersOnly(proveedor)) {
                        contratos = filterTodosByTercodAndTernif(contratos, proveedor);
                    } else {
